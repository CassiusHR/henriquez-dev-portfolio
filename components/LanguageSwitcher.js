...
const AltLangs = ({ altLangs = [], currentLang, inhome, isDesktop }) =>
  altLangs.map((altLang,i) => {
    return (
      <LangLink currentLang={currentLang} inhome={inhome}>
        <Link
          locale={altLang.lang}
          ...
          passHref
          key={`lang-${i}`}
          title={altLang.lang === 'en-us' ? 'Switch to english' : 'Cambiar a español'}
        >
          {altLang.lang === 'en-us' ? <div className='lang-text'>Español</div> : <div className='lang-text'>English</div>}
        </Link>
      </LangLink>
    );
  });
